...
char** str_token(const char* str, const char* delimiter, int* count) {
...
void cmd_redrcn1(char input[]){
	int count_args;
	char **args = str_token(input," ", &count_args);
	char *argv[] = {};
	int m=0;
...
void npipes(char input_string[]){
	...
	int count;
	char** tokens = str_token(input_string, delimiter, &count);
	int fd[2];
	int prev_fd = 0;
		...
		if (pid == 0) 
			{
				...
				char **args = str_token(tokens[i]," ", &count_args);
				char *argss[] = {};
				int k=0;
				while(k<count_args)
					{
